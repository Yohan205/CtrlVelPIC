...
static float freq_ma_buf[8];
static uint8_t ma_idx = 0;
static uint8_t ma_count = 0;
static float freq_ma_sum = 0.0f;
...
void interruptSensor(void){
    ...
    float freq_raw = 1000.0f / (float)contador;

    freq_ma_sum -= freq_ma_buf[ma_idx];
    freq_ma_buf[ma_idx] = freq_raw;
    freq_ma_sum += freq_raw;

    ma_idx++;
    if (ma_idx >= 8) ma_idx = 0;

    if (ma_count < 8) ma_count++;
